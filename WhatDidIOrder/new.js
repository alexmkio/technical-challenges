...
function getOrder(price, menu) {
  ...
  let nextItem = menu.find(currentItem => currentItem.price <= price)
  if (exactMatch) {
    ...
    removed = []
    order.splice(-2, 2)
    getOrder(remainder, menu.sort((a, b) => b.price - a.price))
  } else if (order.length === 1 && removed.length && order[order.length - 1].name === removed[removed.length - 1].name) {
    ...
    menu = [...removed]
    removed = []
    getOrder(price, menu.sort((a, b) => b.price - a.price))
